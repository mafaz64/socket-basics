...
//Function getCurrentUsers which determines the list of users in the room.
function getCurrentUsers(socket) {
	var info = clientInfo[socket.id];
	//Array to hold user names
	var users = [];

	//Check if the info has something
	if (typeof info === 'undefined') {
		return;
	} 

	//Object.keys(clientInfo) returns the array of all the keys in the clientInfo
	...
	socket.emit('message', {
			...
			//Note: The join function on users arrays will conctinate all users the ', ' arg to join will be betwen names
...
io.on('connection', function (socket) {
	...
	socket.on('message', function(message) {
		...
		//Check if the sent message is a command @currentUsers
		if(message.text === '@currentUsers') {
			//Call the function which prints the list of users in the same room.
	...
	//second the data to send. We used object with properties name, timestamp, text.
